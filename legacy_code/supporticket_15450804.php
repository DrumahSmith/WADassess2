...
function GCdisplayfaq() {
    ...
    $query = "SELECT * FROM GC_ticket WHERE status=1 ORDER BY answer_date DESC";
...
//--- end of basic debugging  
	...
	echo  '<div id="msg" style="overflow: auto"></div>
		<div class="wrap">
		<h2>GC 8. Simple FAQ <a href="?page=GCsimplefaq&command=new" class="add-new-h2">Add New</a></h2>
...
function GC_faq_view($id) {
   ...
   $row = $wpdb->get_row("SELECT * FROM GC_ticket WHERE id = '$id'");
   echo '<p>';
   echo "Question:";
   echo '<br/>';
   echo $row->question;
...
function GC_faq_delete($id) {
   ...
   $results = $wpdb->delete('GC_ticket',array( 'id' => $id ) );
   if ($results) {
...
function GC_faq_list() {
   ...
   $query = "SELECT id, question, answer, author_id, answer_date, status FROM GC_ticket ORDER BY answer_date DESC";
   $allfaqs = $wpdb->get_results($query);
...
//if the current command is 'edit' then retrieve the FAQ record based on the id pased to this function
	if ($command == 'update') {
        $faq = $wpdb->get_row("SELECT * FROM GC_ticket WHERE id = '$id'");
	}
