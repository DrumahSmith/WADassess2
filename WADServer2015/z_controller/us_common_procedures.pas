...
begin
 ...
 AProcess.Parameters.Add('/T:1F');                            // Set background colour
...
begin
 ...
 for i:=0 to sList.Count-1 do
   begin
     // APC code
     If ExecRegExpr('^[^;].*'+QuoteRegExprMetaChars(APC_DLL)+'.*$', sList[i]) Then sList[i] := ';'+sList[i];
     // Eaccelerator code
     If ExecRegExpr('^[^;].*'+QuoteRegExprMetaChars(EACCELERATOR_DLL)+'.*$', sList[i]) Then sList[i] := ';'+sList[i];
     // Xcache code
     If ExecRegExpr('^[^;].*'+QuoteRegExprMetaChars(XCACHE_DLL)+'.*$', sList[i]) Then  sList[i] := ';'+sList[i];
     // Zend OpCache code
...
begin
 ...
 // Eaccelerator
 If accelerator = 'eaccelerator' Then
  begin
   for i:=0 to sList.Count-1 do
      begin
         // Eaccelerator code
...
begin
  ...
  // APC code
  If Main.MMSS_php_acc_apc.Checked Then
    Begin
     url := 'http://' + UENV_US_SERVERNAME + ':' + UENV_AP_PORT + '/us_extra/apc.php';
     browser_display_url(url); //Display URL in default/portable browser
    End;

  // Eaccelerator code
  If Main.MMSS_php_acc_eaccelerator.Checked Then
    Begin
     url := 'http://' + UENV_US_SERVERNAME + ':' + UENV_AP_PORT + '/us_extra/eaccelerator_control.php';
     browser_display_url(url); //Display URL in default/portable browser
    End;

  // Xcache code
  If Main.MMSS_php_acc_xcache.Checked  Then
    Begin
     url := 'http://' + UENV_US_SERVERNAME + ':' + UENV_AP_PORT + '/us_extra/xcache/index.php';
     browser_display_url(url); //Display URL in default/portable browser
    End;
